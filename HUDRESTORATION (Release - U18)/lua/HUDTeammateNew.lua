...
function HUDTeammate:set_carry_info(carry_id, value)
	...
	local real_value = managers.money:get_real_job_money_values(num_winners)
...
function HUDTeammate:layout_special_equipments()
	...
	for i, panel in ipairs(special_equipment) do
		if self._main_player and Restoration.options.restoration_bag_style == 1 then
