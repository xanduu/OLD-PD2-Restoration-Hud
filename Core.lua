...
if not json then
	log("[RESTORATION ERROR] JSON library not found! Save/Load disabled.")
end

function Restoration:Save()
	if not json then
		log("[RESTORATION ERROR] Cannot save options: JSON library is missing.")
		return
	end
...
function Restoration:Load()
	if not json then
		log("[RESTORATION ERROR] Cannot load options: JSON library is missing.")
		self.options = {}
		return
	end
	...
	if file then
		...
		if content and content ~= "" then
			self.options = json.decode(content) or {}
		else
			self.options = {}
		end
		file:close()
...
Restoration.dofiles = {
	-- empty, add any extra files here if needed
}
...
if not Restoration.setup then
	if Restoration.Load then
		Restoration:Load()
	else
		log("[RESTORATION ERROR] Load method not available!")
	end
...
--[[
	...
	Hooks:Add("MenuManagerBuildCustomMenus", "MenuManagerBuildCustomMenusRestoration", function(menu_manager, nodes)
		...
		MenuHelper:AddMenuItem(nodes["blt_options"], Restoration.main_menu, "restoration_menu_title", "restoration_menu_desc", 1 )
	end)
end
